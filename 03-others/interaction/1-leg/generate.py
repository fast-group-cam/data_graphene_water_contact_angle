...
if __name__ == '__main__':
    ...
    OH_x = REF_O_H_DIST * np.cos((REF_H_O_H_ANG - 90.0) * np.pi / 180.0)
    OH_z = REF_O_H_DIST * np.sin((REF_H_O_H_ANG - 90.0) * np.pi / 180.0)
    ...
    for z_O in np.linspace(2.5, 4.5, 9):
        ...
        hydrogens = np.array([[x_O, y_O, z_O - REF_O_H_DOWN_DIST], [x_O + OH_x, y_O, z_O + OH_z]], dtype=float)
        ...
        with open(os.path.join(root_folder, sub_folder, 'geometry.in'), 'w') as output_stream:
            for pos in carbons:
                output_stream.write(f'atom   {pos[0]: 10.8f}   {pos[1]: 10.8f}   {pos[2]: 10.8f}   C\n')
            for pos in hydrogens:
                output_stream.write(f'atom   {pos[0]: 10.8f}   {pos[1]: 10.8f}   {pos[2]: 10.8f}   H\n')
            for pos in oxygens:
                output_stream.write(f'atom   {pos[0]: 10.8f}   {pos[1]: 10.8f}   {pos[2]: 10.8f}   O\n')
